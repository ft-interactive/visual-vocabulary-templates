...
const yMax = 8;// sets the maximum value on the xAxis
const yAxisHighlight = 0; // sets which tick to highlight on the yAxis
const yAxisAlign = 'right';// alignment of the axis
const numTicksy = 3;// set the number of scale lines on the y-axis
...
const frame = {
   ...
   .margin({ top: 100, left: 10, bottom: 86, right: 10 })
    // .title("Put headline here")
   .height(600)
   .extend('numberOfColumns', 4)
   .extend('numberOfRows', 2),
...
parseData.load(dataURL).then(({ seriesNames, setColourPalette, groupNames, names, dataSorter, data, groupedData }) => {
    ...
    Object.keys(frame).forEach((frameName) => {
        ...
        myChart
          .yRange([heightOfSmallCharts, 0])
          .xRange([0, widthOfSmallCharts])
