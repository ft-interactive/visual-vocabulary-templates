...
export function load(url, options) { // eslint-disable-line
    return loadData(url).then((result) => {
        ...
        series.forEach((d, i) => {
            valueExtent[0] = Math.min(valueExtent[0], d3.min(d.map((el) => el[0])));
            valueExtent[1] = Math.max(valueExtent[1], d3.max(d.map((el) => el[1])))
        })

        // let min = series.map((d) => {return d3.min(d[0])})
        // let max = series.map((d) => {return d3.max(d[1])})
