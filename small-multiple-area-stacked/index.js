...
const sort = 'ascending';// specify 'ascending', 'descending'
...
const frame = {
    webS: gChartframe.webFrameS(sharedConfig)
        .margin({ top: 10, left: 10, bottom: 88, right: 15 })
    ...
    webL: gChartframe.webFrameL(sharedConfig)
        .margin({ top: 10, left: 10, bottom: 80, right: 15 })
        // .title("Put headline here")
        .height(500)
        .fullYear(true)
        .extend('numberOfColumns', 3)
...
parseData.load(dataFile, { dateFormat, yMin, dataDivisor, sort, sortOn }).then(({ seriesNames, data, plotData, valueExtent, annos }) => {
    Object.keys(frame).forEach((frameName) => {
        ...
        const stackData = d3.stack();
            stackData.keys(seriesNames);
            stackData.order(d3.stackOrderNone);
            stackData.offset(d3.stackOffsetDiverging);
        ...
        chart.selectAll('areas')
            .data((d) => {
                return stackData(data.filter((el) => {return el.category === d.name}))
            })
            .enter()
            .append('g')
            .attr('class', 'areas')
