...
const xMax = 4000;// sets the maximum value on the xAxis
...
.then(({categoryNames, seriesNames, groupNames, plotData, valueExtent, data }) => { // eslint-disable-line
    ...
    Object.keys(frame).forEach((frameName) => {
        ...
        if (yAxisAlign === 'right') {
            const newMargin =  + yAxis0.labelWidth() +  yAxis1.labelWidth() + currentFrame.margin().right;
            // Use newMargin redefine the new margin and range of xAxis
            currentFrame.margin({ right: newMargin });
            yAxis0.yLabel()
                .attr('transform', `translate(${currentFrame.dimension().width + yAxis0.labelWidth() +  yAxis1.labelWidth()},${0})`);
                yAxis1.yLabel()
                .attr('transform', `translate(${currentFrame.dimension().width + yAxis0.labelWidth()},${0})`);
        } else {
            const newMargin = yAxis0.labelWidth() +  yAxis1.labelWidth() + currentFrame.margin().left;
            // Use newMargin re define the new margin and range of xAxis
            currentFrame.margin({ left: newMargin })
             yAxis0.yLabel()
                .attr('transform', `translate(${-yAxis1.labelWidth()},${0})`);;
        }
        ...
        categoryNames.map((d) => {
            ...
            const barHolder = currentFrame.plot()
                .append('g')
                .attr('transform',`translate(0, ${yAxis0.scale()(d)})`)
                .selectAll('.barHolder')
                .data(filteredPlot)
                .enter()
                .append('g')
