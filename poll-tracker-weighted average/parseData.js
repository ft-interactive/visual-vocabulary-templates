...
export function load(url, options) { // eslint-disable-line
    return loadData(url).then((result) => {
        ...
        const { dateFormat, maxAverage, decayDays } = options;
    // return loadData(url).then((result) => {
...
export function getLines(dateExtent, maxAverage, allData, decayDays) {
    ...
    function getAverage(rollinfDate, max, days) {
        const decay = Math.log10(100)/Math.log10(days);
        ...
        poll.forEach(s => {
              ...
              s.weight = Math.max(0,100-Math.pow(daysSince,decay));
            });
        const average = d3.sum(poll.map(d => d.value*d.weight)) / d3.sum(poll.map(d => d.weight));
